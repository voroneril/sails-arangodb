...
var Arango = require('arangojs'),
    ...
    async = require('async'),
    _ = require('lodash'),
    aqb = require('aqb'), //TODO Replace With https://github.com/arangodb/arangojs/blob/master/docs/Drivers/JS/Reference/README.md#aql
  ...
  */
    ...
    async.auto({
        ...
        getNamedGraphs: ['ensureDB', function (next) {
          ...
          .catch((err) => {
            ...
            .finally(function() {
            //https://stackoverflow.com/questions/30552376/asyncnodejs-error-callback-was-already-called
             if(error) {
                callback(error);
             } else {
        ...
        getEdgeCollections: ['ensureDB', function(next) {
          ...
          _.each(collections, function(v, k) {
            var EdgeCollection = {};
            //debug('getEdgeCollections()',v.tableName,  v.tableName.indexOf('__'));
            if(v.tableName.indexOf('__') !== -1) {
                EdgeCollection['edge'] = k;   
                _.each(v.definition, function(vv, kk) {
                if (vv.references) {
        ...
        createMissingCollections: ['getCollections', function(next, results) {
          ...
          debug('currentCollections()', collections);
          var missingCollections = _.filter(collections, function(v, k) {
            //debug('createMissingCollections edgeDefinitions:', v.adapter.query._attributes.$edgeDefinitions.meta);
            ...
            }) === undefined && v.tableName.indexOf('__') === -1 && 
            (!v.definition.$edgeDefinitions || !v.definition.$edgeDefinitions.meta) 
          });
          if (missingCollections.length > 0) {
            async.mapSeries(missingCollections,
              function(collection, cb) {
                debug('db.collection - CALLED', collection.tableName);
                //TODO Add Error if tableName contains '__'
                db.collection(collection.tableName).create(function(err){
                  if (err) {
                    debug('err:', err);
                    return cb(err, null);
                  }
        ...
        createMissingEdges: ['getCollections', 'getEdgeCollections', function(next, results) {
          ...
          var classes = results.getCollections.edgeCollection;
          //debug('getCollections()', results.getCollections, results.getEdgeCollections);
          async.mapSeries(results.getEdgeCollections,
            function(collection, cb) {
              if (!_.find(classes, function(v) {
                  //console.log('FIND', v.name  ,collection.edge)
                  return (v.name == collection.edge);
                })) {
            ...
            function(err, created) {
                debug('Create any missing Edges - DONE');
              next(err, created);
            });
        }],
        ...
        createMissingNamedGraph: ['createMissingCollections', 'getNamedGraphs', function (next, results) {
          ...
          _.each(collections, (v, k) => {
            debug('createMissingNamedGraph hasSchema:', typeof currentNamedGraphs[k] === undefined , v.definition.$edgeDefinitions);
            if (typeof currentNamedGraphs[k] === 'undefined' && v.definition.$edgeDefinitions && v.definition.$edgeDefinitions.meta) {
              missingNamedGraphs[k] = v;
            }
          });
            //console.log("DEBUG missingNamedGraphs", missingNamedGraphs)
          const promises = [];
          _.each(missingNamedGraphs, (g, k) => {
            //console.log("CREATE missingNamedGraphs", g.definition.$edgeDefinitions.meta)
            promises.push(me.createGraph(db, k, g.definition.$edgeDefinitions.meta));
          });
          Q.all(promises)
          .then((r) => {
            next(null, r);
        ...
        addEdgeDefinitions: ['addVertexCollections', 'createMissingEdges', function(next, results) {
  ...
  DbHelper.prototype.optionsToQuery = function(collection, options, qb) {
          ...
          if (_.isArray(v)) {
            ...
            _.each(v, (vv, kk) => {
              ...
              switch(kk) {
                ...
                case 'in':   // in waterline master (upcoming)
                  operator = 'IN';
                  eachFunction = '';
                  break;
    ...
    if (options.where && !_.isEmpty(options.where)) {
      //console.log('function buildWhere 2', options.where);
      var whereStr = '';
    ...
    if (options.sort && !_.isEmpty(options.sort)) {
      ...
      sortArgs = [];
      var force_consistent_results = true;
      options.sort.forEach(function (currentValue, index, array) {
        for (var key in currentValue) {
            var order = currentValue[key]
            sortArgs.push([`d.${key}`, `${order}`]);
            if(key == '_key')
                force_consistent_results = false;
        }
      });
      // force consistent results
      if(force_consistent_results == true)
        sortArgs.push(['d._key', 'ASC']);

      sortArgs = _.flatten(sortArgs);

      debug('sortArgs:', options.sort, sortArgs);
  ...
  DbHelper.prototype.count = function(collection, options, cb) {
      //RETURN LENGTH(collection)
    var me = this;
    debug('connection count(1) collection:', collection, 'options:', options);
    /*
    var is_edge = false;
    me.edgeCollections.forEach(function (el) {
        if(el.edge == collection) {
            is_edge = true;
        }
    })
    for (var key in me.collections) {
        if (me.collections[key].definition.$edgeDefinitions && me.collections[key].definition.$edgeDefinitions.meta) {
            if(key == collection) {
                is_edge = true;
            }
        }
    ...
    if(is_edge) {
        debug('is_edge find(+) collection');
        return cb(null, 0); //FIX ME!
    }
    */
    //debug('connection find(3) collection:', collection, 'is_edge:', is_edge);
    //RETURN {d: d} -> RETURN LENGTH(d: d)
    var qb = this.optionsToQuery(collection, options);
    qb = me.applyFunctions(options, qb);

    var find_query = qb.toAQL().replace('RETURN {d: d}', 'RETURN LENGTH(d)');
    
    debug('count_query:', find_query);

    this.db.query(find_query, function(err, cursor) {
      debug('connection count() query err:', err);
      if (err) return cb(err);

      me._filterSelected(cursor, options)
      .then((vals) => {
        debug('query count response :', vals.length, 'documents returned for query:', find_query);
        debug('count reuslt:', vals);
  ...
  DbHelper.prototype.find = function(collection, options, cb) {
    var me = this;
    debug('connection find(1) collection:', collection, 'options:', options);
    /*
    var is_edge = false;
    me.edgeCollections.forEach(function (el) {
        if(el.edge == collection) {
  ...
  DbHelper.prototype.destroy = function(collection, options, cb) {
    //FixME Maybe delete related graph connections?
    var qb = this.optionsToQuery(collection, options);
    debug('destroy() qb:', qb);
  ...
  DbHelper.prototype.createEdge = function(from, to, edge_name, options, cb) {
    ...
    var edge_name = srcAttr.edge;*/
    
    // create edge
    this.graph.edgeCollection(edge_name,
   ...
   */
  DbHelper.prototype.deleteEdges = function(from, to, edge_name, options, cb) {
    var src = toArangoRef(from),
      dst = toArangoRef(to);
