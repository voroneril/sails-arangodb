...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerDatastore: function(connection, collections, cb) {
      debug('registerDatastore() connection:', connection);
     ...
     */
    find: function(connectionName, query, cb) {
      debug('adaptor find() connectionName:', connectionName, 'collectionName:', query.using, 'searchCriteria:', query.criteria);
        //console.log(new Error().stack);
      connections[connectionName].find(query.using, query.criteria, function (err, r) {
        /* istanbul ignore if */
        if (err) {
          return cb(err);
        }
        ...
        var cast_r = processor.cast(query.using, {_result: r});
...
`;
    ...
    */
    ...
    create: function(connectionName, query, cb) {
      var collectionName = query.using;
      var data = query.newRecord;
      ...
      return col.save(data, true, function (err, doc) {
        ...
        var processor = new Processor(connections[connectionName].collections);
        debug('create err:', err, 'returning doc 1:', doc);
        debug('create err:', err, 'returning doc.new:', processor.cast(collectionName, {_result: doc.new}));
     ...
     * @param  {String}       connectionName The name of the datastore to perform the query on.
     * @param  {Dictionary}   query         The stage-3 query to perform.
     * @param  {Function}     cb            Callback
     */
    createEach: function createEach(connectionName, query, cb) {
      var collectionName = query.using;
      var data = query.newRecords;
      data = _.map(data, function(r) {if(typeof r._key != 'undefined') delete r._key; return r;});
      debug('createEach() collectionName:', collectionName, 'data:', data);
      var col = connections[connectionName].db.collection(collectionName);
      return col.save(data, true, function (err, doc) {
        /* istanbul ignore if */
        if (err) {
        ...
        var processor = new Processor(connections[connectionName].collections);
        debug('create err:', err, 'returning doc:', doc);
        if (query.meta && query.meta.fetch) {
          ...
          //}
          doc = _.map(doc, function(r) {return r.new;});
          return cb(null, processor.cast(collectionName, {_result: doc}));
        }
        return cb();
      });
     ...
     * @param   {function} cb             Callback (err, data)
     */
    update: function(connectionName, query, cb) {
      var collectionName = query.using;
      var searchCriteria = query.criteria;
      var values = query.valuesToSet;
      ...
      .update(collectionName, searchCriteria, values, function (err, docs) {
        ...
        if (query.meta && query.meta.fetch) {
            ...
            return cb(null, processor.cast(collectionName, {_result: docs}));
        }
        return cb(null);
      });
    },
     ...
     * @param  {Function}     cb            Callback
     */
    destroy: function(connectionName, query, cb) {
      var collectionName = query.using;
      var options = query.criteria;
      (function maybeFetchRecords(done) {

        if (query.meta && query.meta.fetch) {
          adapter.find(connectionName, _.cloneDeep(query), function(err, records) {
          ...
          .destroy(collectionName, options, (err, docs) => {
            ...
            var processor = new Processor(connections[connectionName].collections);
            debug('destroy err:', err, 'returning records:', records);
...
*/
     ...
     * @param   {function} cb         Callback (err, data)
     */
    join: function(connection, query, cb) {
      //console.log('join query', query)
      var collection = query.using;
      var criteria = query;
    ...
    count: function(connectionName, query, cb) {
      debug('adaptor count() connectionName:', connectionName, 'collectionName:', query.using, 'searchCriteria:', query.criteria);
        //console.log(new Error().stack);
      connections[connectionName].count(query.using, query.criteria, function (err, r) {
        /* istanbul ignore if */
        if (err) {
          return cb(err);
        }
